import DailyPass from "./pass/Daily.js";
import MonthyPass from "./pass/Monthly.js";
import YearlyPass from "./pass/Yearly.js";
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .getElementById("registerationButton")
    .addEventListener("click", handleUser);
  document
    .getElementById("vehicleRegisterationButton")
    .addEventListener("click", handleVehicle);

  document.querySelectorAll("form h1").forEach((item) =>
...
const handleUser = (e) => {
  ...
  if (userName && userContact) {
    ...
    let user =
      role === "student"
        ? new Student(userName, userContact)
        : new Faculty(userName, userContact);
    alert(
      `Congratulations !! You are successfully registered as ${role}. Your registeration id is ${user.id}`
    );
    users.push(user);
...
const updatePassChoice = (vehicle) => {
  ...
  for (const key in vehicle.getPriceOptions()) {
    ...
    button.addEventListener("click", (e) => {
      createPass(e, vehicle);
    });
    passChoice.appendChild(button);
  }
};

const createPass = (e, vehicle) => {
  let pass = null;
  switch (e.target.value.split(":")[0].trim()) {
    case "daily":
      pass = new DailyPass(vehicle);
      break;
    case "monthly":
      pass = new MonthyPass(vehicle);
      break;
    case "yearly":
      pass = new YearlyPass(vehicle);
      break;
  }
